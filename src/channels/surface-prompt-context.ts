...
export function buildSurfacePromptContext(params: {
	...
	return {
		...
		scheduledLoop: params.scheduledLoopId
			? { id: params.scheduledLoopId }
			: undefined,
	};
}
...
export function renderSenderPromptText(
	...
	const details = [
		sender.senderId,
		sender.handle ? `@${sender.handle}` : undefined,
	]
		.filter(Boolean)
		.join(", ");
...
export function renderSurfacePromptText(
	...
	const platform = surface.surfaceId.startsWith("slack:")
		...
		: surface.surfaceId.startsWith("telegram:")
			...
			: "Terminal";
	if (surface.parent) {
		const parentKind = surface.parent.surfaceId.includes(":group:")
			? "group"
			: "channel";
		const childKind = surface.surfaceId.includes(":thread:")
			? "thread"
			: surface.kind;
		return [
...
export function renderSurfacePromptContext(context: SurfacePromptContext) {
	return [
		...
		...(context.scheduledLoop
			? [`- message: scheduled loop ${context.scheduledLoop.id}`]
			: []),
	].join("\n");
}
