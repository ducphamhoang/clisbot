...
async function readDirectory(pathname: string): Promise<SurfaceDirectoryShape> {
	...
	const parsed = JSON.parse(
		await readTextFile(pathname),
	) as Partial<SurfaceDirectoryShape>;
	return {
		version: 1,
...
function upsertSurface(
	...
	directory.surfaces[surface.surfaceId] = {
		...
		parentSurfaceId:
			surface.parent?.surfaceId ?? existingSurface?.parentSurfaceId,
		updatedAt: now,
	};
}
...
export async function buildSurfacePromptContextWithDirectory(params: {
	...
	try {
		const directory = await readDirectory(
			resolveDirectoryPath(params.stateDir),
		);
		return {
			...context,
