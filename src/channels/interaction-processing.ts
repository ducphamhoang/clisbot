...
import {
	logLatencyDebug,
	type LatencyDebugContext,
} from "../control/latency-debug.ts";
...
function renderNewSessionFailureMessage(error: unknown) {
	const details =
		error instanceof Error && error.message.trim()
			? error.message.trim()
			: "Unknown error.";
	return ["Could not finish opening a new runner conversation.", details].join(
		"\n",
	);
}
...
function renderWhoAmIMessage(params: {
	...
	if (
		...
		params.sessionDiagnostics.storedSessionId !==
			params.sessionDiagnostics.sessionId
	) {
		lines.push(
			`storedSessionId: \`${params.sessionDiagnostics.storedSessionId}\``,
		);
	}
...
function renderRouteStatusMessage(params: {
	...
	if (params.runtimeState.startedAt) {
		lines.push(
			`run.startedAt: \`${new Date(params.runtimeState.startedAt).toISOString()}\``,
		);
	}

	if (params.runtimeState.detachedAt) {
		lines.push(
			`run.detachedAt: \`${new Date(params.runtimeState.detachedAt).toISOString()}\``,
		);
	}
...
function renderResponseModeStatusMessage(params: {
	...
	if (params.persisted) {
		...
		lines.push(
			`config.responseMode: \`${params.persisted.responseMode ?? "(inherit)"}\``,
		);
	}
...
function renderStreamingStatusMessage(params: {
	...
	const lines = [`Streaming mode: \`${params.route.streaming}\``];
...
async function executePromptDelivery<TChunk>(params: {
	...
	const paneManagedDelivery =
		params.route.responseMode === "capture-pane" ||
	...
	async function recordVisibleReply(
		...
		if (kind === "final") {
			...
			await params.agentService.recordConversationReply(
	...
	async function getMessageToolRuntimeSignals() {
		...
		const runtime = await params.agentService.getSessionRuntime?.(
			params.sessionTarget,
		);
		return {
	...
	async function waitForMessageToolFinalReply(
		params: { deadlineAt?: number; stopWhen?: () => boolean } = {},
	) {
		const deadline =
			params.deadlineAt ?? Date.now() + MESSAGE_TOOL_FINAL_GRACE_WINDOW_MS;
	...
	async function maybeRenderQueueStartNotification() {
		...
		const text = renderQueueStartNotification({
			...
			promptSummary:
				...
				summarizeSurfaceNotificationText(
					params.queueText ?? String(params.promptText),
				),
		});
		if (!text) {
	...
	try {
			...
			params.promptText,
			{
				...
				onUpdate: async (update) => {
					...
					await (renderChain = renderChain.then(async () => {
						...
						if (
							messageToolPreview &&
							hasMessageToolFinalReplyBoundary(initialSignals)
						) {
							await handoffMessageToolPreview();
							return;
						}
						...
						const nextState = buildRenderedMessageState({
							...
							maxChars: Number.POSITIVE_INFINITY,
							note: update.note,
							allowTranscriptInspection: allowTranscriptInspectionForRoute(
								params.route,
							),
		...
		const earlyToolFinalOutcome = returnOnToolFinal
			? await Promise.race([
					...
					waitForMessageToolFinalReply({
						...
						seen ? ("tool-final" as const) : ("result" as const),
					),
				])
			: "result";
		...
		if (
			params.suppressDetachedSettlement &&
			finalResult.status === "detached"
		) {
			return;
		}
			...
			const activeText =
				error.update.status === "detached"
					? resolveDetachedInteractionNote({
							...
							transcriptCommand:
								params.identity.platform === "telegram"
									? "/transcript"
									: "`/transcript`",
						})
...
export async function processChannelInteraction<TChunk>(params: {
	...
	const auth = params.auth ?? {
		principal: params.senderId
			? `${params.identity.platform}:${params.senderId}`
			: undefined,
		appRole: "member",
		agentRole: "member",
	...
	const sessionBusy = await (params.agentService.isAwaitingFollowUpRouting?.(
		params.sessionTarget,
	) ??
	...
	const queueByMode =
		...
		params.route.additionalMessageMode === "queue" &&
		sessionBusy;
	const forceQueuedDelivery =
		typeof explicitQueueMessage === "string" || queueByMode;
	...
	const queueItem = forceQueuedDelivery
		? createStoredQueueItem({
				...
				promptSummary:
					explicitQueueMessage ?? summarizeQueuePrompt(delayedPromptQueueText),
	...
	if (slashCommand?.type === "control") {
		if (slashCommand.name === "start" || slashCommand.name === "status") {
			const followUpState =
				await params.agentService.getConversationFollowUpState(
					params.sessionTarget,
				);
			const runtimeState = await params.agentService.getSessionRuntime(
				params.sessionTarget,
			);
			const sessionLoops =
				params.agentService.listIntervalLoops?.({
					sessionKey: params.sessionTarget.sessionKey,
				}) ?? [];
			await params.postText(
				renderRouteStatusMessage({
					...
					loopState: {
						sessionLoops,
						globalLoopCount:
							params.agentService.getActiveIntervalLoopCount?.() ?? 0,
					},
					timezone: {
		...
		if (slashCommand.name === "transcript") {
			...
			const transcript = await params.agentService.captureTranscript(
				params.sessionTarget,
			);
			await params.postText(
		...
		if (slashCommand.name === "attach") {
			...
			if (!observation.active || !observation.update) {
				await params.postText(
					"This thread does not have an active run to attach to.",
				);
				await params.agentService.recordConversationReply(params.sessionTarget);
				return interactionResult;
			}
		...
		if (slashCommand.name === "watch") {
			...
			if (!observation.active || !observation.update) {
				await params.postText(
					"This thread does not have an active run to watch.",
				);
				await params.agentService.recordConversationReply(params.sessionTarget);
				return interactionResult;
			}
		...
		if (slashCommand.name === "new") {
			...
			try {
				const rotated = await params.agentService.triggerNewSession(
					params.sessionTarget,
				);
				await params.postText(
					[
		...
		if (slashCommand.name === "nudge") {
			const nudged = await params.agentService.nudgeSession(
				params.sessionTarget,
			);
			await params.postText(
				nudged.nudged
		...
		if (slashCommand.name === "followup") {
			if (slashCommand.action === "status") {
				...
				await params.agentService.resetConversationFollowUpMode(
					params.sessionTarget,
				);
				await params.postText(
		...
		if (slashCommand.name === "queue-list") {
			const queuedItems = await (params.agentService.listQueuedPrompts?.(
				params.sessionTarget,
			) ?? []);
		...
		if (slashCommand.name === "queue-clear") {
			const clearedCount = await (params.agentService.clearQueuedPrompts?.(
				params.sessionTarget,
			) ?? 0);
			await params.postText(
				clearedCount > 0
	...
	if (slashCommand?.type === "loop-control") {
		...
		if (slashCommand.all) {
			const cancelled = await params.agentService.cancelIntervalLoopsForSession(
				params.sessionTarget,
			);
			await params.postText(
				cancelled > 0
	...
	if (slashCommand?.type === "loop") {
		...
		const effectiveIntervalMs =
			slashCommand.params.mode === "interval"
				? slashCommand.params.intervalMs
				: undefined;
		...
		let resolvedLoopPrompt: Awaited<
			ReturnType<typeof resolveManagedLoopPromptText>
		>;
		try {
		...
		if (slashCommand.params.mode === "calendar") {
			...
			const createdLoop = await params.agentService.createCalendarLoop({
				...
				promptSource: resolvedLoopPrompt.maintenancePrompt
					? "LOOP.md"
					: "custom",
	...
	if (explicitSteerMessage) {
		const hasActiveRun =
			params.agentService.hasActiveRun?.(params.sessionTarget) ?? false;
		if (!hasActiveRun) {
		...
		await params.agentService.submitSessionInput(
			...
			buildSteeringPromptText({
				text:
					params.transformSessionInputText?.(explicitSteerMessage) ??
					explicitSteerMessage,
				identity: params.identity,
				agentId: params.route.agentId,
				time: Date.now(),
	...
	await executePromptDelivery({
		...
		notificationPromptSummary:
			explicitQueueMessage ?? summarizeSurfaceNotificationText(params.text),
		queueItem,
		postText: params.postText,
		reconcileText: params.reconcileText,
		observerId,
