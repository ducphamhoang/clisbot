...
function resolveSlackFollowUpModeTarget(
	...
	if (params.identity.conversationKind === "dm") {
		const targetId =
			params.identity.senderId?.trim() || params.identity.channelId?.trim();
		if (!targetId) {
			throw new Error(
				"Slack follow-up channel scope requires a senderId or channelId.",
			);
		}
		...
		const existingRoute =
			...
			(bot.directMessages[routeKey] =
				createDirectMessageBehaviorOverride(wildcardRoute));
		return {
	...
	if (!route) {
		throw new Error(
			`Route not configured yet: slack group:${channelId}. Add the route first.`,
		);
	}
...
function resolveTelegramFollowUpModeTarget(
	...
	if (params.identity.conversationKind === "dm") {
		const targetId =
			params.identity.senderId?.trim() || params.identity.chatId?.trim();
		if (!targetId) {
			throw new Error(
				"Telegram follow-up channel scope requires a senderId or chatId.",
			);
		}
		const routeKey = targetId;
		const wildcardRoute = resolveDirectMessageWildcardRoute(bot.directMessages);
