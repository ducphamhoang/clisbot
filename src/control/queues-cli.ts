import { buildStoredLoopSender } from "../agents/loop-control-shared.ts";
import {
...
import {
	resolveLoopCliContext,
	type LoopCliContext,
} from "./loop-cli-context.ts";
...
function normalizeTelegramTarget(
	...
	if (addressing.target.startsWith("topic:")) {
		...
		return {
			...addressing,
			target: chatId,
			topicId: addressing.topicId ?? topicId,
		};
	}
	return addressing;
}
...
function parseQueueCliAddressing(args: string[]): QueueCliAddressing {
	if (
		parseOptionValue(args, "--surface") ||
		parseOptionValue(args, "--session-key")
	) {
		throw new Error(
	...
	if (addressing.channel === "slack" && addressing.topicId) {
		throw new Error(
			"Slack queue commands use `--thread-id`, not `--topic-id`.",
		);
	}
...
function resolveScopedContext(
	...
	if (!addressing.channel || !addressing.target) {
		throw new Error(
			"--channel and --target are required for scoped queue commands.",
		);
	}
	return resolveLoopCliContext({
		...
		threadId:
			addressing.channel === "telegram" ? undefined : addressing.threadId,
		topicId:
			addressing.topicId ??
...
function parseQueueSender(
	...
	if ((platform !== "slack" && platform !== "telegram") || !providerId) {
		throw new Error(
			"--sender must be a principal like telegram:<id> or slack:<user-id>.",
		);
	}
	if (addressing.channel && platform !== addressing.channel) {
		throw new Error(
			`--sender platform must match --channel ${addressing.channel}.`,
		);
	}
...
function assertQueueSenderMatchesContext(
	...
	if (senderPlatform && senderPlatform !== context.channel) {
		throw new Error(
			`--sender platform must match target channel ${context.channel}.`,
		);
	}
}
...
function resolveProtectedControlMutationRule(
	...
	return auth.mayManageProtectedResources
		? undefined
...
async function sendQueueCreatedNotificationToSurface(
	...
	const plugin = listChannelPlugins().find(
		(entry) => entry.id === params.context.channel,
	);
	if (!plugin) {
		throw new Error(
			`Unsupported queue notification channel: ${params.context.channel}`,
		);
	}
...
async function listQueues(
	...
	const context =
		addressing.channel || addressing.target
...
async function createQueue(
	...
	const text = renderQueueCreatedNotification({
		queueId: item.id,
		positionAhead,
		promptText,
	});
	await deps
		...
		.catch((error) => {
			deps.warn(
				`Queued prompt ${item.id}, but surface acknowledgement failed: ${String(error)}`,
			);
		});
	deps.print(
		`Queued prompt \`${item.id}\` for \`${context.sessionTarget.sessionKey}\`.`,
	);
}
...
async function clearQueues(
	...
	const sessionKey = context.sessionTarget.sessionKey;
	const cleared =
		await state.sessionState.clearPendingQueuedItemsForSessionKey(sessionKey);
	deps.print(
...
export async function runQueuesCli(
	...
	if (command === "list" || command === "status") {
		await listQueues(
			state,
			parseQueueCliAddressing(args.slice(1)),
			command,
			deps,
		);
		return;
