...
import {
  ...
  renderStartSummary,
} from "../runtime/runtime-summary.ts";
import {
...
async function applyBootstrapStateToConfig(params: {
  ...
  const lifecycleLines =
    commandName === "start" && !runtimeRunning
      ? deactivateExpiredMemBots(config, collectLiteralBootstrapBotIds(bootstrapFlags))
      : [];
