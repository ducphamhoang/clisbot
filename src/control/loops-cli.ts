...
function requireLoopContext(addressing: LoopCliAddressing) {
	if (!addressing.channel || !addressing.target) {
		throw new Error(
			"--channel and --target are required for scoped loop commands.",
		);
	}
}
...
function resolveScopedLoopContext(
	...
	if (addressing.channel === "telegram" && addressing.threadId) {
		throw new Error(
			"Telegram loop commands use `--topic-id`, not `--thread-id`.",
		);
	}
...
function resolveScopedLoopCancelId(
	args: string[],
	sessionLoops: IntervalLoopStatus[],
) {
	const explicitLoopId = stripLoopContextArgs(args.slice(1)).find(
		(token) => token && token !== "--all" && token !== LOOP_APP_FLAG,
	);
	return (
		explicitLoopId ||
...
async function executeCountLoop(params: {
	...
	try {
		for (let index = 0; index < params.count; index += 1) {
			while (true) {
				...
				try {
					await agentService.enqueuePrompt(
						params.context.sessionTarget,
						builtPrompt,
						{
							onUpdate: () => undefined,
						},
	...
	console.log(
		`Completed ${params.count} iteration${params.count === 1 ? "" : "s"}.`,
	);
}
...
function parseCreateExpression(
	rawArgs: string[],
	...
	if (!expression) {
		throw new Error(
			"Loop creation requires an interval, count, or schedule expression.",
		);
	}
	return expression;
}
...
function parseLoopCreator(
	...
	if ((platform !== "slack" && platform !== "telegram") || !providerId) {
		throw new Error(
			`${LOOP_SENDER_FLAG} must be a principal like telegram:<id> or slack:<user-id>.`,
		);
	}
	if (addressing.channel && platform !== addressing.channel) {
		throw new Error(
			`${LOOP_SENDER_FLAG} platform must match --channel ${addressing.channel}.`,
		);
	}
	...
	if (!creator) {
		throw new Error(
			`${LOOP_SENDER_FLAG} must include a non-empty provider id.`,
		);
	}
	return creator;
}
...
function requireValidIntervalLoop(
	parsed: Extract<ParsedLoopSlashCommand, { mode: "interval" }>,
) {
...
async function resolveLoopCreateRequest(
	...
	const deliveryContext =
		surfaceChannelId && surfaceChannelId !== context.identity.channelId
			...
			: undefined;
	const resolvedTarget = resolveAgentTarget(
		state.loadedConfig,
...
function buildRecurringLoopPromptMetadata(request: LoopCreateRequest) {
	return {
		...
		loopStart:
			request.parsed.mode === "times" ? undefined : request.parsed.loopStart,
...
function buildRecurringLoopFirstRunNote(
...
async function createCalendarLoop(base: LoopCreateBase) {
	...
	await base.state.sessionState.setIntervalLoop(
		base.request.resolvedTarget,
		loop,
	);
	...
	console.log(
		renderLoopStartedMessage({
			...
			firstRunNote: buildRecurringLoopFirstRunNote(
				"calendar",
				base.runtimeRunning,
			),
			...counts,
...
async function hasSuccessfulCalendarLoop(state: LoadedLoopControlState) {
	return (await state.sessionState.listIntervalLoops()).some(
		(loop) => loop.kind === "calendar",
	);
}
...
function renderCalendarConfirmation(params: {
	...
	const timezoneClause = params.request.loopTimezone
		? ` --timezone ${params.request.loopTimezone}`
		: "";
	...
	return [
		...
		`timezone: ${params.timezone}`,
		nextRunAt
			? `next run: ${new Date(nextRunAt).toISOString()}`
			: "next run: unknown",
		"",
