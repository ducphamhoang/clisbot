...
function shouldDropDeliveryReportLine(line: string) {
  ...
  return (
    /^(?:I|We)\s+(?:have\s+)?(?:sent|posted|delivered)\b.*\b(?:channel|chat|conversation|thread)\b/i.test(normalized) ||
    /^(?:Sent|Posted|Delivered)\b.*\b(?:to|via)\s+(?:[A-Z][\w-]*(?:\s+Bot)?|channel|chat|conversation|thread)\b/.test(normalized) ||
    /^Đã gửi\b.*\b(?:channel|chat|conversation|thread|[A-Z][\w-]*(?:\s+Bot)?)\b/.test(normalized) ||
