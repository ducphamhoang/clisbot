...
| Active | runner | pi | [runners](runners/README.md) | [runner tasks](../tasks/features/runners) | 2026-05-26 | Pi (pi.dev by Earendil Works) registered as a supported AI coding CLI alongside codex, claude, and gemini; tmux runner with explicit session UUID, startup blockers for missing models and extended-keys, active-timer pattern, and pi-chrome filtering in transcript normalization. |
| Active | ops | setup wizard | [setup-wizard](../user-guide/setup-wizard.md) | — | 2026-05-28 | Interactive readline wizards for first-run operator setup: `clisbot setup channels` (Flow A, token collection), `clisbot setup agent` (Flow B, CLI and workspace), and `clisbot setup` smart router that auto-detects what is missing. `clisbot start` now warns and continues in unrouted mode when channels are configured but no agent is set. |
