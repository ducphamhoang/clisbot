...
Add a real-CLI smoke surface that can run a small scenario suite against Codex, Claude, and Gemini, save artifacts, classify failures, and output a backend-by-backend compatibility summary.
...
The operator needs a way to validate real upstream CLIs directly and quickly, without first wiring a full fake backend harness.
...
- one smoke command for a single backend and scenario
- one `--backend all --suite launch-trio` roll-up mode
- one artifact directory per run
- one JSON result object per run
- one summary view that grades each backend by capability strength
...
- [Backend Profiles](../../../features/dx/cli-compatibility/backend-profiles.md)
